...
3. **Data Pipelines:** Tự động hóa quá trình Tiền xử lý (Vá dữ liệu thiếu bằng `SimpleImputer`, Chuẩn hóa bằng `StandardScaler`, và Biến đổi biến phân loại bằng `OneHotEncoder`).
...
- **LinearRegression:** 68,675 USD
- **DecisionTreeRegressor:** 67,734 USD
- **RandomForestRegressor:** 48,304 USD (Nhà vô địch)

Sau khi ép xung Random Forest bằng `GridSearchCV` (cv=5) và cho đi thi thật trên tập Test nguyên sơ, AI đạt được:
- **Sai số RMSE Cuối cùng: 50,437 USD**

## Hạn chế và Hướng phát triển tương lai

### Hạn chế hiện tại
- **Kỹ nghệ đặc trưng (Feature Engineering) cơ bản:** Mô hình hiện đang sử dụng các đặc trưng thô có sẵn trong tập dữ liệu. Các tỷ lệ quan trọng như số phòng trên mỗi hộ gia đình hay số phòng ngủ trên tổng số phòng chưa được khai thác.
...
- **Không gian tinh chỉnh (Hyperparameter Tuning) hẹp:** Lưới tìm kiếm (GridSearch) mới chỉ được thiết lập với một số lượng nhỏ các tham số nhằm tiết kiệm tài nguyên và thời gian huấn luyện mô hình.

### Hướng phát triển tương lai
- **Bổ sung Custom Transformers:** Viết thêm các lớp tiền xử lý để tự động kết hợp và đẻ ra các đặc trưng mới (ví dụ: `rooms_per_household`, `population_per_household`) trực tiếp bên trong Pipeline.
- **Nâng cấp thuật toán:** Triển khai thử nghiệm thêm Gradient Boosting Regressor hoặc XGBoost để cố gắng tối ưu hóa và giảm sai số RMSE xuống dưới mức 45,000 USD.
